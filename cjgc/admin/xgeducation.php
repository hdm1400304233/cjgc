...
{  
//print_r($_POST);		     
$sql="UPDATE `education` SET weizhi='$_POST[weizhi]',title='$_POST[title]', content='$_POST[content]',money='$_POST[money]' WHERE `id`='$_POST[id]'";
mysql_query($sql);
echo "<script language='javascript'>alert('保存成功');</script>";
echo "<script language='javascript'>location.href=ededucation.php;</script>";										
}   
if(isset($_GET['id']))
{
	$result=mysql_query('SELECT * FROM `education` WHERE `id`=\''.$_GET['id'].'\'');
	$row=mysql_fetch_array($result);
	;

?>
<body>
<center><form action="xgeducation.php" method="post" name="myform" onsubmit="return chk();">
...
<center><a href="ededucation.php">返回</a></center>
</body>
</html>