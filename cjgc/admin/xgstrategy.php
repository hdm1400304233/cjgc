...
{  
//print_r($_POST);		     
$sql="UPDATE `strategypl` SET weizhi='$_POST[weizhi]',title='$_POST[title]', content='$_POST[content]',money='$_POST[money]' WHERE `id`='$_POST[id]'";
mysql_query($sql);
echo "<script language='javascript'>alert('保存成功');</script>";
echo "<script language='javascript'>location.href=edstrategypl.php;</script>";										
}   
if(isset($_GET['id']))
{
	$result=mysql_query('SELECT * FROM `strategypl` WHERE `id`=\''.$_GET['id'].'\'');
	$row=mysql_fetch_array($result);
	;

?>
<body>
<center><form action="xgstrategy.php" method="post" name="myform" onsubmit="return chk();">
...
<center><a href="edstrategypl.php">返回</a></center>
</body>
</html>