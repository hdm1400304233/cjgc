...
{  
//print_r($_POST);		     
$sql="UPDATE `transport` SET weizhi='$_POST[weizhi]',title='$_POST[title]', content='$_POST[content]',money='$_POST[money]' WHERE `id`='$_POST[id]'";
mysql_query($sql);
echo "<script language='javascript'>alert('保存成功');</script>";
echo "<script language='javascript'>location.href=edtransport.php;</script>";										
}   
if(isset($_GET['id']))
{
	$result=mysql_query('SELECT * FROM `transport` WHERE `id`=\''.$_GET['id'].'\'');
	$row=mysql_fetch_array($result);
	;

?>
<body>
<center><form action="xgtransport.php" method="post" name="myform" onsubmit="return chk();">
...
<center><a href="edtransport.php">返回</a></center>
</body>
</html>